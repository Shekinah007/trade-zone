...
export default function AdminMonetizationPage() {
  ...
  const [featuredTiers, setFeaturedTiers] = useState<any[]>([]);
  ...
  const [newFeaturedTier, setNewFeaturedTier] = useState({
    name: "",
    durationInDays: 0,
    priceNGN: 0,
    creditCost: 0,
    ...
    try {
      ...
      if (settingsRes.ok) {
        ...
        if (data.settings) {
          ...
          setGlobalListingExpiryDays(
            data.settings.globalListingExpiryDays || 60,
          );
          setMaxFeaturedSlots(data.settings.maxFeaturedSlots || 5);
        }
      }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  Configure credit costs, listing quotas, and boosts
                </p>
              </div>
            </div>
            <button
            ...
            <div className="bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <form onSubmit={handleSaveSettings} className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      Unlimited Reg Price (NGN)
                    </label>
                    <input
                      type="number"
                      value={unlimitedRegistrationPriceNGN}
                      onChange={(e) =>
                        setUnlimitedRegistrationPriceNGN(Number(e.target.value))
                      }
                      className="w-full text-sm p-2 bg-gray-50 border rounded-lg"
                    />
                  </div>
                ...
                <div className="pt-4 border-t">
                  <button
                    disabled={saving}
                    type="submit"
                    className="w-full py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-lg flex justify-center items-center gap-2 shadow-lg"
                  >
                    {saving ? (
            ...
            <div className="bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                {listingPacks.map((pack) => (
                  <div
                    key={pack._id}
                    className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border"
                  >
                    <div>
                      <p className="font-medium text-sm">{pack.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {pack.slotCount} slots • ₦{pack.priceNGN} •{" "}
                ...
                <div className="grid grid-cols-2 gap-3 mt-4 border-t pt-4">
                  ...
                  <button
                    ...
                    className="col-span-2 py-2 bg-blue-600 text-white rounded flex items-center justify-center gap-2 text-sm"
                  >
              ...
              <div className="p-6 space-y-4">
                  ...
                  {boostTiers.map((tier) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {tier.durationInDays} days • ₦{tier.priceNGN} •{" "}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 border-t pt-4 max-w-3xl">
                  ...
                  <button
                    ...
                    className="col-span-2 md:col-span-4 py-2 bg-amber-600 text-white rounded flex items-center justify-center gap-2 text-sm"
                  >
                    <Plus className="w-4 h-4" /> Add Boost Tier
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden lg:col-span-2">
              <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-gradient-to-r from-purple-50/50 to-transparent">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold">Featured Tiers</h3>
                </div>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {featuredTiers.map((tier) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => handleDeleteFeaturedTier(tier._id)}
                        className="text-red-500 p-2 hover:bg-red-50 rounded-lg"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4 border-t pt-4 max-w-3xl">
                  ...
                  />
                  <button
                    onClick={handleAddFeaturedTier}
                    className="col-span-2 md:col-span-4 py-2 bg-purple-600 text-white rounded flex items-center justify-center gap-2 text-sm"
                  >
                    <Plus className="w-4 h-4" /> Add Featured Tier
                  </button>
                </div>
              </div>
            </div>
