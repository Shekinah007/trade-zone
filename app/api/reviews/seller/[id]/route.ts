...
export async function GET(
  ...
  try {
    ...
    const stats = await Review.aggregate([
      { $match: { reviewee: params.id } }, // Match strictly by string ID if stored that way, or ObjectId if casted. Mongoose aggregate usually needs ObjectId if stored as such.
      // However, params.id is string. Let's cast it securely.
    ]);
    
    // Simpler find first to avoid aggregation complexity issues blindly
    const reviews = await Review.find({ reviewee: params.id })
