...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { tokenType, quantity, batchName } = body;
    ...
    if (!quantity || quantity < 1 || quantity > 5000) {
      return NextResponse.json({ error: 'Quantity must be between 1 and 5000' }, { status: 400 });
    }

    const value = tokenType === 'basic' ? 10 : -1;
    ...
    const csvRows = createdTokens.map(t => `${t.code},${t.tokenType},${t.value},${t.expiresAt.toISOString()}`).join('\n');
    ...
    return NextResponse.json({ error: error.message || 'Error generating tokens' }, { status: 500 });
  }
}
