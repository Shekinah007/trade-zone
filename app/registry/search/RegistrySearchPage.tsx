...
export default function RegistrySearchPage() {
  ...
  // locating: true while geolocation is in-flight. Search waits for this to
  // become false before firing so coords are always included if available.
  const [locating, setLocating] = useState(true);
  const pendingQuery = useRef<string | null>(null);
  const searchedQuery = useRef<string | null>(null);

  // On mount kick off geolocation immediately. When it resolves (or fails),
  // locating flips to false and the effect below fires the pending search.
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setLocating(false); // no coords — search will proceed without them
      },
      { timeout: 5000, maximumAge: 60000 },
    );
  }, []);

  // Capture the query from URL params. Don't search yet — just store it.
  useEffect(() => {
    const q = searchParams.get("q");
    if (q && searchedQuery.current !== q) {
      setQuery(q);
      pendingQuery.current = q; // held until locating is done
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams]);

  // Fire the search once locating is done (coords resolved or timed out).
  // This guarantees coords are included when available.
  useEffect(() => {
    if (locating) return; // still waiting — do nothing
  ...
  const doSearch = async (
    q: string,
    activeCoords: { lat: number; lng: number } | null = coords,
  ) => {
    ...
    try {
      ...
      if (activeCoords) {
        url += `&lat=${activeCoords.lat}&lng=${activeCoords.lng}`;
      }
        ...
        <div className="container mx-auto px-4 max-w-4xl">
            ...
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-white/80 backdrop-blur-sm border border-slate-200 shadow-sm">
              ...
              >
                {locating ? (
                  ...
                  <>
                    <MapPin className="h-3 w-3" /> Location on
                  </>
                ) : (
                  <>
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-600 max-w-2xl mx-auto leading-relaxed">
              ...
              using IMEI, serial numbers, or chassis identifiers.
            </p>
          </div>
          ...
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSubmit} className="relative group">
              ...
              >
                <div
                  className={`absolute inset-0 rounded-3xl bg-linear-to-r from-red-500 to-purple-600 opacity-0 transition-opacity duration-300 blur-xl ${focused ? "opacity-20" : ""}`}
                />
                ...
                <div className="relative flex items-center gap-2 p-2">
                  ...
                  <div className="flex-shrink-0 pr-2">
                    {locating ? (
                      ...
                      <div
                        title={`Location active: ${coords.lat.toFixed(3)}, ${coords.lng.toFixed(3)}`}
                      ...
                      <div
                        title="Location unavailable — results won't be location-sorted"
          ...
          <div className="flex flex-col items-center justify-center py-24">
            ...
            {coords && (
              <p className="text-sm text-emerald-600 mt-1 flex items-center gap-1.5">
                <MapPin className="h-3.5 w-3.5" /> Using your location for
                nearby results
              </p>
            )}
          </div>
        ...
        {!loading && searched && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-600 text-center max-w-md mb-8 leading-relaxed">
              This identifier isn't registered yet. Register your property to
              protect it against theft and simplify recovery.
            </p>
            <div className="flex gap-3 flex-wrap justify-center">
              ...
              >
                <Link href="/registry/register">
                  <Shield className="mr-2 h-5 w-5" /> Register This Item
                </Link>
              </Button>
              <Button
                asChild
            ...
            <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1 flex items-center gap-1.5">
                  {coords ? (
                    <>
                      <MapPin className="h-3.5 w-3.5 text-emerald-500" /> Sorted
                    ...
                    <div className="space-y-3 flex-1">
                      ...
                      {session && item.owner ? (
                        ...
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                          <Lock className="h-3.5 w-3.5" /> Owner details hidden
                          — sign in to view
                        </div>
                      ) : null}
                    </div>
