...
export default function UserTokensPage() {
  ...
  // Dynamic import on click — window is always available at this point
  const handleBuyPackPaystack = async (pack: any) => {
    setPurchasingPackId(pack._id);
    setPurchasingMethod("paystack");

    try {
      const { usePaystackPayment } = await import("react-paystack");
      // usePaystackPayment is a hook so we can't call it here directly.
      // Use the lower-level PaystackButton approach or inline-js instead.
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800/50">
                  {tokens.map((t: any) => (
                    <tr
                      key={t._id}
                      className="hover:bg-gradient-to-r hover:from-emerald-50/30 hover:to-red-50/30 dark:hover:from-emerald-950/20 dark:hover:to-red-950/20 transition-all duration-200"
                    >
                      ...
                      <td className="px-6 py-4">
                        {t.tokenType === "unlimited" ? (
                          <Badge className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 border-none shadow-sm">
                            <Infinity className="w-3 h-3 mr-1" /> Unlimited
                            Granted
                          </Badge>
                        ) : (
                          <Badge
