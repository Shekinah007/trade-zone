...
import { Loader2, Eye, EyeOff, ShieldCheck, Zap, Globe } from "lucide-react"
...
export default function SignUpPage() {
  ...
  return (
    <div className="grid lg:grid-cols-2 h-[calc(100vh-4rem)] overflow-hidden">
      {/* Left panel — branding */}
      <div className="hidden lg:flex flex-col justify-between bg-primary p-12 text-primary-foreground relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-8">
          {[
            { icon: Zap, title: "List in seconds", desc: "Post your item and reach thousands of buyers instantly." },
            { icon: ShieldCheck, title: "Safe & verified", desc: "Our approval system ensures only trusted sellers operate." },
            { icon: Globe, title: "Local community", desc: "Connect with buyers and sellers near you." },
            ...
            <div key={title} className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-primary-foreground/70 mt-0.5">{desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Right panel — form */}
      <div className="flex items-center justify-center px-6 py-6 bg-background">
        <div className="w-full max-w-md space-y-6">
          {/* Header */}
          <div className="text-center lg:text-left">
            <Link href="/" className="lg:hidden text-xl font-extrabold tracking-tight text-primary mb-1 block">
                    ...
                    {password.length > 0 && (
                      <div className="flex gap-3 mt-1 flex-wrap">
                        {strength.map(({ label, met }) => (
                          <span key={label} className={`text-xs flex items-center gap-1 ${met ? "text-green-500" : "text-muted-foreground"}`}>
