...
import { zodResolver } from "@hookform/resolvers/zod";
import * as z from "zod";
import { Loader2, Upload, X, ImagePlus, Tag, DollarSign, MapPin, FileText, Info } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import {
...
export function ListingForm({ initialData, categories }: ListingFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const existingImages = imageItems.filter((i) => !i.isNew).map((i) => i.url);
      existingImages.forEach((url) => formData.append("existingImages", url));

      imageItems.filter((i) => i.isNew && i.file).forEach((i) => {
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Title</FormLabel>
                  <FormControl>
                    <Input placeholder="e.g. iPhone 14 Pro Max 256GB Space Black" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center gap-1.5">
                    Unique Identifier
                    <span className="text-xs text-muted-foreground font-normal">(Serial No., VIN, IMEI, etc.)</span>
                  </FormLabel>
                  <FormControl>
                    <Input placeholder="e.g. SN123456789" {...field} />
                  </FormControl>
