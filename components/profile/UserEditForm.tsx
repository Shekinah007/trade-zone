...
export function UserEditForm({ initialData }: UserEditFormProps) {
  ...
  const takePhoto = () => {
    ...
    // Mirror front camera so the shot isn't flipped
    if (facingMode === "user") {
    ...
    <>
      ...
      {cameraOpen && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col">
          ...
          <div className="flex-1 relative flex items-center justify-center bg-black overflow-hidden">
            {cameraError ? (
              <div className="text-center px-8 space-y-4">
                <ZapOff className="h-12 w-12 text-red-400 mx-auto" />
            ...
            {!cameraError && (
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-8 left-8 w-12 h-12 border-t-2 border-l-2 border-white/60 rounded-tl-lg" />
                <div className="absolute top-8 right-8 w-12 h-12 border-t-2 border-r-2 border-white/60 rounded-tr-lg" />
                <div className="absolute bottom-8 left-8 w-12 h-12 border-b-2 border-l-2 border-white/60 rounded-bl-lg" />
                <div className="absolute bottom-8 right-8 w-12 h-12 border-b-2 border-r-2 border-white/60 rounded-br-lg" />
              </div>
            )}
          </div>
          ...
          {/* Shutter */}
          {!cameraError && (
            <div className="flex items-center justify-center py-8 bg-black/60 backdrop-blur-sm">
              <button
                onClick={takePhoto}
                className="w-16 h-16 rounded-full bg-white border-4 border-white/30 shadow-xl hover:scale-95 active:scale-90 transition-transform"
              />
            </div>
          )}
        </div>
